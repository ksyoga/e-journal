...
<html>
  ...
  <body class="hold-transition login-page">
      ...
      <div class="row">
        ...
        <section class="content-header" style= "padding:0;">
          <h4>
            ...
            @else
              {{$instrument->brand}} {{$instrument->model}} Total Station Utilization<small> by {{Auth::user()->name}} </small>
            @endif
          </h4>
        </section>

        <table class="table table-condensed">
            <tr @if($instrument->iutilize()->first() != null)class="user{{$surveyor_id}}"@endif>
              <th>Date</th>
              <th>Surveyor</th>
              <th>Requisition</th>
              <th>Days</th>
            ...
            @foreach($instrument->iutilize()->orderBy('used_date','desc')->get() as $iutilize)
               <tr class="user{{$iutilize->requisition->surveyor->id}}">
    ...
    <script>
      ...
      @endif
      @if($instrument->iutilize()->first() == null)
        $(".table-condensed  tr").hide();
      @endif
    </script>
  </body>
